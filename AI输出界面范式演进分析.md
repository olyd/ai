...
MCP Apps 的核心思想是：让任何第三方服务都能在 AI 对话中弹出自己的交互界面。技术机制是：MCP 服务器在响应时附带一个 `ui://` 资源地址，AI 客户端把这个地址加载到安全沙箱 iframe 中渲染，双方通过 JSON-RPC 实时通信。它是 MCP—Server 侧基于 MCP 协议、通过 Resources 能力单元向 Host 暴露的动态 UI 资源包，可让 Server 直接定义交互界面规则，无需 Host 预开发适配组件即可实现交互式工具的标准化集成与多端复用
